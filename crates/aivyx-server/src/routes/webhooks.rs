...
pub async fn receive_webhook(
    ...
    let trigger = config
        ...
        .ok_or_else(|| {
            ServerError(aivyx_core::AivyxError::Config(format!(
                "webhook trigger not found: {trigger_name}"
            )))
        })?
        .clone();
    drop(config);
    ...
    if let Some(ref secret_ref) = trigger.secret_ref {
        let signature = headers
            ...
            .ok_or_else(|| {
                ServerError(aivyx_core::AivyxError::CapabilityDenied(
        ...
        let secret_bytes = enc_store
            ...
            .ok_or_else(|| {
                ServerError(aivyx_core::AivyxError::Config(format!(
                    "webhook secret not found: {secret_ref}"
                )))
            })?;
        ...
        let expected_bytes = hex::decode(expected).map_err(|_| {
            ServerError(aivyx_core::AivyxError::CapabilityDenied(
    ...
    let prompt = trigger
        .prompt_template
        .replace("{payload}", &sanitized_payload);
